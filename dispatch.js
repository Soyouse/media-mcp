...
// Middleware : compte les réponses invalides (401/403/429) par provider → observabilité quota/auth.
...
export function monitorInvalidResponses(next) {
  return async (args, ctx) => {
    try {
      ...
      if (typeof e?.status === "number") recordResult(args?.provider, e.status);
      throw e;
    }
  };
}
...
export async function handleTool(name, args, session = {}) {
  ...
  //    entre les appels d'UNE session, jamais entre sessions) → provider par-session, zéro fuite.
