/**
 * Media MCP — transport HTTP (StreamableHTTP) pour le service 24/7 distant.
 *
 ...
 *   - Auth Bearer OBLIGATOIRE : le serveur REFUSE de démarrer sans MEDIA_MCP_HTTP_TOKEN.
...
const TOKEN = process.env.MEDIA_MCP_HTTP_TOKEN;
const HOST = process.env.MEDIA_MCP_HTTP_HOST || "127.0.0.1";
const PORT = Number(process.env.MEDIA_MCP_HTTP_PORT || 8791);

if (!TOKEN) {
  process.stderr.write("FATAL: MEDIA_MCP_HTTP_TOKEN manquant — refus de démarrer sans auth.\n");
  process.exit(1);
}
...
if (HOST === "0.0.0.0" && process.env.MEDIA_MCP_CONTAINER !== "1") {
  process.stderr.write(
    "FATAL: bind 0.0.0.0 hors conteneur interdit — 127.0.0.1 + tunnel Tailscale, ou MEDIA_MCP_CONTAINER=1.\n"
  );
  process.exit(1);
}

const app = express();
app.use(express.json({ limit: "25mb" })); // images/vidéos base64 possibles dans les payloads → limite large
...
const EXTRA_HOSTS = (process.env.MEDIA_MCP_ALLOWED_HOSTS || "")
  .split(",")
  .map((h) => h.trim())
  .filter(Boolean);
const ALLOWED_HOSTS = [
...
app.listen(PORT, HOST, () => {
  process.stderr.write(`Media MCP HTTP sur http://${HOST}:${PORT}/mcp (Bearer requis)\n`);
});
